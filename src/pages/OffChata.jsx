...
function OffChats() {
    ...
    useEffect(() => { setJ(true) }, [isChat])
    return (
        <>
            <div style={{ display: "flex", height: "calc(100vh - 55px)", width: "100%", margin: "0px auto", overflow: "hidden", backgroundColor: "white", border: "1px solid black" }}>
                            ...
                            chats?.map((e) => {
                                ...
                                return (
                                    <div key={e._id} onClick={() => {
                                        ...
                                        setIsChat(e)
                                    }} style={{ border: e.isReplied ? null : "1px solid green", background: e.isReplied ? "var(--bg-white)" : "#b6ffb6", cursor: "pointer", alignItems: "center", minHeight: "60px", cursor: "pointer", display: "flex", gap: "10px", borderRadius: "6px", padding: "6px 15px", width: "90%" }}>
                                        ...
                                        <div>
                                            <p>{e.user.name}</p>
                                            <p>{e.createdAt.split("T").slice(0, 1)} , {new Date(e.createdAt).getHours()}:{new Date(e.createdAt).getMinutes()}</p>
                                        </div>
                                    </div>
                                )
                            })
                ...
                {isChat._id ?
                    <div style={{ borderLeft: "1px solid gray", height: "100%", width: "100%", }}>
                        <div style={{ width: "100%", height: "70px", display: "flex", position: "sticky", top: "0px", justifyContent: "space-between", padding: "0px 20px", backgroundColor: "var(--cta-white)", textTransform: "capitalize" }}>
                            <div onClick={() => setUserProfile("profile")} style={{ cursor: "pointer", color: "black", display: "flex", alignItems: "center", gap: "20px" }}>
                                <div style={{ height: "50px", aspectRatio: "1", borderRadius: "50%", background: "red", overflow: "hidden" }}>
                                    <img src={isChat?.user?.avatar?.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                </div>
                                <div>
                            ...
                            <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
                                {
                                    !isChat.isReplied && j && <div onClick={() => {
                                        const a = window.confirm("Do You Want To Finish The Chat")
                                        a && dispatch(StopChat({ astroId: astro._id, userId: isChat.user._id, offline: true, offChatId: isChat._id })).then((e) => {
                                            e?.payload?.success && setChats(e.payload.offChat)
                                            e?.payload?.success && setIsChat({})

                                            setJ(false)
                                        })
                                    }} style={{ borderRadius: "10px", fontWeight: "700", background: "var(--yellow)", padding: "10px 20px", cursor: "pointer" }}>
                                        Finish
                                    </div>
                                }
                        ...
                        <div style={{ height: "calc(100% - 70px)", padding: "50px", backgroundColor: "var(--bg-white)", width: "100%", overflow: "scroll", }}>
                            <div style={{ display: "flex", flexDirection: "column", backgroundColor: "var(--white)", justifyContent: "flex-end", gap: "3px", minHeight: "100%", border: "1px solid gray", borderRadius: "10px" }}>
                                ...
                                <div style={{ width: "100%", display: "flex", padding: "10px 20px", gap: "10px", backgroundColor: "var(--bg-white)", textTransform: "capitalize" }}>
                                    <input value={content}
                                        onKeyDown={(e) => {
                                            if (content) {
                                                if (e.key === "Enter") {
                                                    dispatch(astroOffChat({ astroId: astro._id, userId: astro._id, offChatId: isChat._id, content })).then((e) => {
